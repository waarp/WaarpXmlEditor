...
import javax.swing.*;
import org.w3c.dom.*;
import org.waarp.xample.custnodes.*;
import org.apache.xml.serialize.OutputFormat;
import org.apache.xml.serialize.XMLSerializer;
import com.fg.util.FLoader;
import com.fg.util.FadingFilter;
import com.fg.xmleditor.*;

/**
 * A simple application demonstrates usage of the Swing-based XML editing component
 * {@link com.fg.xmleditor.FXDoubleView}. The component generates document-specific GUI for each
 * particular type of the XML document, specified by the XML Schema. A User-Manual document, which
 * is included into XAmple XML Editor distribution package, provides detailed description of how to
 * customize XML Schema documents with the editor-specific features, such as custom field editors,
 * additional labeling, node-specific messages, etc.
 * <p>
 * </p>
 * The application creares an instance of the {@link org.waarp.xample.custnodes.FFileDialog} and
 * registers it with the {@link com.fg.xmleditor.FXDoubleView} under "FileDialog" id, so it can be
 * used as a global field editor. (Actually there is no particular sense in doing it since the
		...
		implements ActionListener, ItemListener, FXModelStatusListener
{
	...
	public XAmple()
	{
		...
		btnOpenXML = new FHistoryButton(icoOpenXML, "Open XML Configuration Document",
		...
		for (int i = 0; i < lfi.length; i++)
		{
			if (lfi[i].getName().equals(lfName))
			{
				try {
					...
					javax.swing.filechooser.FileFilter currFilter = fileDlg.getFileFilter();
					fileDlg.setAcceptAllFileFilterUsed(fileDlg.getAcceptAllFileFilter() == null);
	...
	void newXML()
	{
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "Can't open new XML Configuration Document",
	...
	boolean loadXMLSchema(File file)
	{
		try {
			...
			btnOpenXSD.setItems(history.items);
			btnOpenXML.setItems(childHistory.items);
			dblView.showInfoMessage("XML Configuration Schema: " + file.getAbsolutePath());
			return true;
		} catch (Exception ex) {
			...
			JOptionPane.showMessageDialog(this, "Can't load the XML Configuration Schema",
					"Error", JOptionPane.ERROR_MESSAGE);
			setTitle("XAmple");
			return false;
		}
	...
	boolean loadXMLDocument(File file)
	{
		...
		try {
			...
			childHistory.put(file.getAbsolutePath());
			btnOpenXML.setItems(childHistory.items);
			dblView.showInfoMessage("XML Configuration Document: " + file.getAbsolutePath());
			return true;
		} catch (Exception ex) {
			...
			setXMLEnabled(false);
			dblView.showErrorMessage("Error: " + ex.getMessage());
			JOptionPane.showMessageDialog(this, "Can't load the XML Configuration Document",
					"Error", JOptionPane.ERROR_MESSAGE);
			showXSDTitle();
			return false;
		}
